...
class LoRaDecoder():
    def __init__(self, sf, plen=16, cp=7) -> None:
    ...
    def decode_from_raw(self, fileName):
        raw = LoRaDecoder.read_complex(fileName)
        raw = raw[::2]
        symbols, loc_start = self.demodulate(raw)
        decoded = self.decode(symbols)

        return loc_start, symbols, decoded[:self.payload_len]
    ...
    def demodulate(self, raw):
        ...
        # plt.figure()
        # plt.plot(np.abs(np.fft.fftshift(np.fft.fft(raw[loc_start:loc_start+self.samp_per_symbol]*self.downchirp))))
        # plt.show()
        ...
        cfo_correction = np.exp(-1j*2*np.pi*self.cfo*(1/self.fs)*np.arange(self.samp_per_symbol))

        # plt.figure()
        # plt.plot(np.abs(np.fft.fftshift(np.fft.fft(raw[loc_start:loc_start+self.samp_per_symbol]*cfo_correction*self.downchirp))))
        # plt.show()
        # print(raw[loc_start:loc_start+10])
        # print(chirp_list[:10])

        # reshape s.t. each row is a chirp
        chirp_list = chirp_list.reshape(-1, self.samp_per_symbol)

        # print(chirp_list.shape)
        # print(chirp_list[0][:10])

        # print(cfo_correction.shape)
        # print(self.downchirp.shape)

        # temp = chirp_list*cfo_correction*self.downchirp
        # plt.figure()
        # plt.plot(abs(np.fft.fftshift(np.fft.fft(temp, axis=1))[0]))
        # plt.show()
        chirp_list = chirp_list*cfo_correction*self.downchirp

        fftData = np.zeros_like(chirp_list)

        for i in range(fftData.shape[0]):
            fftData[i] = abs(np.fft.fft(chirp_list[i]))
        
        # print(fftData.shape)


        # fftData = np.fft.fftshift(np.fft.fft(chirp_list*cfo_correction*self.downchirp, axis=1))

        # print(fftData.shape)

        # plt.figure()
        # plt.plot(abs(fftData[0]))
        # plt.show()

        # print(fftData.shape)
        symbols = np.argmax(fftData, axis=1)
        # print(symbols)
        # print(symbols, type(symbols[0]))
        # print(symbols.shape)
        return symbols, loc_start


    def start_detection(self, raw):
        '''use the preamble and sync to find the start of the packet
        instead use the correlation (simultanously cumulate data and complex noise), 
        multiple raw data with 8 down chirps (signal becomes the power while the complex noise is still noise.)
        Since the netid is unkown, use zeros to replate that. 
        '''
        preamble_original = np.matlib.repmat(self.downchirp, 1, self.preamble_len).flatten()
        ...
        preamble = np.concatenate((preamble_original, netid, sync))
        max_val = 0
        loc_time = 0
        loc_freq = 0
        for i in range(len(raw)-len(preamble)):
            temp_fft = abs(np.fft.fftshift(np.fft.fft(raw[i:i+len(preamble)]*preamble)))
        ...
        # temp_fft = abs(np.fft.fft(raw[loc_time:loc_time+len(preamble_original)]*preamble_original))
        # temp_freq = np.argmax()
        
        # loc_freq %= self.num_bins
        # print(loc_time)
        # print('original cfo:', loc_freq)
        loc_freq -= np.floor(len(preamble)/2)
        # print('cfo minus half preamble:', loc_freq, '/', len(preamble))
        # loc_freq = 
    ...
    def gray_coding(self, data):
        '''gray coding to make adjacent numbers has only one bit flip, 
        so that even the fft peak is slightly offset to adjacent number (1 bit flip), 
        it can be fixed by the hamming code.
        '''
        # header
    ...
    def whiten(self, data_bytes):
        '''whitening process: data (uint8) xor whitening sequence
        '''
        return np.array([x^y for x,y in zip(data_bytes, self.whitening_seq)]).astype(np.uint8)
    

    def calc_symbol_num(self):
        # calculate how many symbols needed for that much payload
...
if __name__ == '__main__':
    ...
    phy = LoRaDecoder(sf, plen, cp)
    loc_start, symbols, decoded = phy.decode_from_raw(fileName)
    print(loc_start)
    # print("".join(symbols))
    print(symbols)
    print(decoded)
